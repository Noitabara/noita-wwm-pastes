...
export class WeightBuff extends StatusEffect {
    ...
    @Override
    ontick(): void {
        const locPalyersData: IWeightPasteData = Pastes.INST.buff_weight_data[this.entity.asPlayer!.identifier]
        ...
        this.entity.asPlayer?.notifyStat(StatNotificationType.Zero, effectTickAmount)
        locPalyersData.PasteBuffTick++
    }
    @Override
    shouldPass(): boolean {
        const locPalyersData: IWeightPasteData = Pastes.INST.buff_weight_data[this.entity.asPlayer!.identifier]
        const buffCalc: number = Math.floor(locPalyersData.PasteBuffMaxDura / 10) * 2
